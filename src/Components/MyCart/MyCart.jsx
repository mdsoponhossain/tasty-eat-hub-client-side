...
const MyCart = () => {
    ...
    useEffect(() => {
        fetch(`https://tasty-eats-hub-server-51lgk2gqk-sopon.vercel.app/userCart/${user?.email}`)
            .then(res => res.json())
            .then(data => {
                console.log(data)
                setAddCarts(data)
    ...
    const deleteHandle = (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`https://tasty-eats-hub-server-51lgk2gqk-sopon.vercel.app/userCart/${id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
    ...
    return (
        <div>
            <h3 className="text-xl text-center font-semibold my-3">Added to Cart:{addCarts.length} </h3>
